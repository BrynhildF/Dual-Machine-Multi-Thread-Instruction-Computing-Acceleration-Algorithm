...
DWORD WINAPI ThreadProc_sum(LPVOID lpParameter)
{
    ...
    const float* p = rawFloatData + startIndex;    // AVX批量处理时所用的指针.
    ...
    floatResults[thread_ID] = q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];

    ReleaseSemaphore(hSemaphores[thread_ID], 1, NULL);//释放信号量，信号量加1 
    return 0;
}
...
float sumSpeedUp(const float data[], const int len) {
    ...
    //收割
    floatSum = 0.0f;
    size_t datanum_per_AVX256 = 8;    // 块宽
    size_t Multipledata_num = THREADS_NUM / datanum_per_AVX256;   // 块数.
    __m256 Multiple_sum = _mm256_setzero_ps();   // 求和变量
    __m256 Multiple_load;    // 加载.
    const float* p = floatResults;    // AVX批量处理时所用的指针.
    const float* q;    // 将AVX变量上的多个数值合并时所用指针.
    for (size_t i = 0; i < Multipledata_num; ++i)
    {
        Multiple_load = _mm256_load_ps(p);
    ...
    q = (const float*)&Multiple_sum;
    floatSum = q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
...
DWORD WINAPI ThreadProc_max(LPVOID lpParameter) {
    int thread_ID = *(int*)lpParameter;
    int startIndex = thread_ID * NUMperTHREAD;
    const size_t datanum_per_AVX256 = 8;    // 块宽
    const size_t Multipledata_num = NUMperTHREAD / datanum_per_AVX256;   // 块数

    // 数据处理
    __m256 Multiple_load;    // 加载.
    const float* pp = rawFloatData + startIndex;// AVX批量处理时所用的指针.
...
float maxSpeedUp(const float data[], const int len) {
    ...
    const size_t datanum_per_AVX256 = 8;    // 块宽
    size_t cntBlock2 = THREADS_NUM / datanum_per_AVX256;   // 块数
    __declspec(align(32)) float output[8];
    __m256 Multiple_load2;
    const float* pp = floatMaxs;// AVX批量处理时所用的指针.
...
DWORD WINAPI ThreadProc_sort(LPVOID pM) {
    ...
    // 数据处理
    __m256 Multiple_sortdata[Multipledata_num];   // 转换后的数组
    __m256 Multiple_load;    // 加载.
    const float* pp = rawFloatData + startIndex;// AVX批量处理时所用的指针.

    for (size_t i = 0; i < Multipledata_num; ++i)
    {
        Multiple_load = _mm256_load_ps(pp);    // [AVX] 加载
    ...
    //把q里面的数据放到starIndex到endIndex的数组里
    ...
    ReleaseSemaphore(hSemaphores_sort[thread_ID], 1, NULL);//释放信号量，信号量加1 
    return 0;
}
...
DWORD WINAPI ThreadProc_join(LPVOID lpParameter) {
    ...
    ReleaseSemaphore(hSemaphores_join[thread_ID], 1, NULL);//释放信号量，信号量加1 
    return 0;
}
