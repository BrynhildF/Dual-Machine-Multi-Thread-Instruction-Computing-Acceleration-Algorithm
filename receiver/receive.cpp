...
*/ 
    ...
    for (size_t i = 0; i < DATANUM; i++)//取后640万
    {
        rawFloatData[i] = float((rand() + rand() + rand() + rand()));
    }
    ...
    addr.sin_addr.s_addr = inet_addr("169.254.21.243"); //target PC
    addr.sin_port = htons(8081); //server Port 端口号
    ...
    if (newConnection == 0)
    {
        ...
        if (receiveSorts[DATANUM + 1] > MaxResult)
            MaxResult = receiveSorts[DATANUM + 1];
        SumResult = SumResult + receiveSorts[DATANUM + 0];
